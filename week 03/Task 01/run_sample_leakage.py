...
log("Querying Sample Table (June 2026) Features and Labels...")
query = """
WITH june_features AS (
    SELECT 
        client_hash_id, 
        content_hash_id,
...
FROM june_features f
LEFT JOIN june_labels l ON f.client_hash_id = l.client_hash_id AND f.content_hash_id = l.content_hash_id
...
WHERE m.is_published IS TRUE 
  ...
  AND (DATE '2026-06-15' - m.content_created_date) >= 90
"""
