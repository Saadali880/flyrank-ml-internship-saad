...
log(f"Step 1: Querying June 1-15 Features for {target_client}...")
q_feat = f"""
SELECT 
    client_hash_id, 
    content_hash_id,
...
log(f"Step 2: Querying June 16-30 Labels for {target_client}...")
q_label = f"""
SELECT 
    client_hash_id, 
    content_hash_id,
...
FROM read_parquet('{fact_sample_url}')
WHERE client_hash_id = '{target_client}'
  AND report_date BETWEEN DATE '2026-06-16' AND DATE '2026-06-30'
GROUP BY client_hash_id, content_hash_id
"""
